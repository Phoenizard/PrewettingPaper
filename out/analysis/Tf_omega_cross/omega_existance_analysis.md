...
溶质 2 的墙面增强相互作用由负变正时，两个趋势同时出现：线明显变长，同时贴上 binodal，最靠近处已经与之接触。溶质 1 的墙面增强相互作用只给出靠拢的趋势，线的长短几乎不受影响，而线到 binodal 的距离仍单调缩短。两者的差别可以从表面能读出：吸附力对墙面上的溶质含量是一次的，增强相互作用是二次的，后者只有在墙面已经聚集了该溶质时才显著。这里所取的吸附力使墙面富集溶质 2、贫化溶质 1，因此只有溶质 2 的增强相互作用有足够大的墙面含量可以作用；溶质 1 的增强相互作用改变不了薄膜态与厚膜态共存的组成范围，只能把这段范围整体移近 binodal。

与吸附力的差别在于线伸展的方向，这一点在两者共同覆盖的组成范围内可以直接比出来。吸附力最强的一例与增强相互作用最强的一例沿溶质 1 方向覆盖的范围几乎相同，但后者沿溶质 2 方向覆盖得明显更宽，并把线推到更高的溶质 2 含量。越出这一范围之后，吸附力驱动的线继续沿溶质 1 伸展，而溶质 2 的含量不再上升。两者放宽 prewetting 条件的方式因此不同：吸附力放宽的是溶质 1 的远场含量范围，增强相互作用放宽的是溶质 2 的。这与增强相互作用所作用的对象一致——它改变的是溶质 2 在墙面的含量，需由远场补偿的也是溶质 2。