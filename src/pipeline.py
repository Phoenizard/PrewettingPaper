...
def _solve_line(cfg, source, i, n, fixed):
    """One scan line, self-contained for a worker process. Lines are
    independent (seed continuation lives inside a line), so distributing
    them over processes changes no numbers — mirrors the reference codes'
    in-case parallelism."""
    ...
    log(f"{label} line done in {time.time() - t_line:.1f}s -> {len(crossings)} crossing(s)")
    return crossings


def _filter_points(crossings, fixed, source, demixed_map, cfg):
    kept = []
    for eq in crossings:
...
def _scan_direction(cfg, demixed_map, source, fixed_values, line_workers=1):
    n = len(fixed_values)
    log(
        f"[{source}] direction start, {n} lines x {cfg.scan.n_scan} points x 2 branches, "
        f"line_workers={line_workers}"
    )
    t_dir = time.time()

    if line_workers <= 1:
        per_line = [_solve_line(cfg, source, i, n, float(v)) for i, v in enumerate(fixed_values)]
    else:
        per_line = [None] * n
        with ProcessPoolExecutor(max_workers=int(line_workers)) as pool:
            futures = {
                pool.submit(_solve_line, cfg, source, i, n, float(v)): i
                for i, v in enumerate(fixed_values)
            }
            for future in as_completed(futures):
                per_line[futures[future]] = future.result()

    rows = []
    for i, fixed in enumerate(fixed_values):
        kept = _filter_points(per_line[i], fixed, source, demixed_map, cfg)
        rows += [(source, p1, p2) for p1, p2 in kept]
    log(f"[{source}] direction done in {(time.time() - t_dir) / 60:.1f} min, {len(rows)} points")
    return rows


def run_case(cfg, out_dir, max_lines=None, line_workers=1):
    ...
    rows = _scan_direction(cfg, demixed_map, "fix_phi2", fixed_values, line_workers=line_workers)
    rows += _scan_direction(cfg, demixed_map, "fix_phi1", fixed_values, line_workers=line_workers)
