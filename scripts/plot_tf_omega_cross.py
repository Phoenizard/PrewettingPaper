"""Publication and supplementary T-f wall-affinity figures from archived data."""
...
COLORS = [viridis(t) for t in (0.06, 0.27, 0.50, 0.73, 0.94)]
MARKERS = ["o", "s", "^", "D", "P"]
PW_COLOR = "#b2182b"
...
def load_case(root, om1, om2):
    directory = root / CHI_DIR / om_dir(om1, om2) / CHIBB_DIR
    px, py = read_cols(directory / "pw_line.csv", ["phi1_inf", "phi2_inf"])
    return {"om1": om1, "om2": om2, "px": px, "py": py}
...
def main_figure(branches, row1, row2, lim, output):
    ...
    for ax, cases, key, index in ((axes[0], row1, "om1", 1),
                                  (axes[1], row2, "om2", 2)):
        ...
        for case, color, marker in zip(cases, COLORS, MARKERS):
            ax.scatter(case["px"], case["py"], s=15, marker=marker,
                       ...
                       label=rf"$\omega_{index}={case[key]:g}$", zorder=3)
        style_axes(ax, lim)
        ax.legend(loc="best", frameon=False, handletextpad=0.3,
                  borderaxespad=0.2, labelspacing=0.25)
    axes[0].set_ylabel(r"$\phi_{2,\infty}$")
    axes[0].set_title(r"varying $\omega_1$; $\omega_2=-0.375$")
    axes[1].set_title(r"varying $\omega_2$; $\omega_1=0.25$")
...
def supplementary_figure(branches, row1, row2, lim, output):
    ...
    for row, cases, key, index in ((axes[0], row1, "om1", 1),
                                   (axes[1], row2, "om2", 2)):
        for ax, case, color, marker in zip(row, cases, COLORS, MARKERS):
            ...
            style_axes(ax, lim)
            ax.set_title(rf"$\omega_{index}={case[key]:g}$")
    for ax in axes[0]:
        ax.set_xlabel("")
...
def main():
    ...
    row1 = [load_case(root, value, CENTRE_OM2) for value in ROW1_OM1]
    row2 = [load_case(root, CENTRE_OM1, value) for value in ROW2_OM2]
    branches = load_gapless_binodal(Path(args.binodal_tie))
    lim = limits(branches, row1 + row2)
    main_paths = main_figure(branches, row1, row2, lim, out / "omega_cross_main.png")
    supp_paths = supplementary_figure(branches, row1, row2, lim,
                                      out / "omega_cross_supplement_2x5.png")
    plt.close("all")
    print("main:", *main_paths)
    print("supplement:", *supp_paths)
    for case in row1 + row2:
        print(f"omega=({case['om1']:g}, {case['om2']:g}) n_pw={len(case['px'])}")
