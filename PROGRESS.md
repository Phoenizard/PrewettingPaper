...
- 验证路线：改走参考代码交叉审计。reference/ 现有两份代码（ternary_ref_code、
  prewetting_project_clean），静态交叉审计已完成，结论见
  doc/note/code_cross_comparison.md：同模型、同数值主干，真差异为 thick 初猜振幅
  与每线点数；两码生产参数点不重叠。
...
验证路线已调整：逐 case 复现（约 20 min/case）与加速均已搁置，改走两份参考代码
交叉审计路线（见 2026-07-08 交叉审计条目）。静态审计已完成，
后续方向由用户决定：可选项包括同点复算（同一 (chi, omega, chibb) 两码各扫一线
逐点对比，需服务器）、或基于审计结论直接采信 unvalidate_data。

（原重写路线存档：以 reference/ 为基准逐项对齐数值与逻辑，先复现其结果再谈适配；
由用户主导决定起步点，Claude 不擅自开写。）
