...
- 阶段：代码库已重建并完成 12 case 验证与人工比对。结论：已确认找到正确的实验代码
  路线，本地代码库具备独立复现 unvalidate_data 的能力——12 个 case 中 8 个与对照图
  一致；4 个有显著差异，且差异已归因到一个参数歧义（见下），不是模型或求解器错误。
- 参数歧义（已标记，待裁决）：thick 初猜振幅在两份参考代码中对同一参数设置不同——
  ternary_ref_code 生产用 0.09/0.02（产出 T-b/T-d/T-f 数据），prewetting_project_clean
  生产用 0.3/0.3（产出 T-a/T-c/T-e 数据）。我们全局取 0.3/0.3，导致 ternary 产的
  stage 上厚支冷启动整线失败（T-f 两 case 0 点、T-b 一 case 线中段缺失）：大振幅
  种子在 omega 含正号（贫化打穿可行域下界）或 chi12=-8.5 的拓扑上首点即死，
  续种子级联全灭。机制细节与证据（fails=400/400、dropped=0）见 2026-07-08 深夜日志。
- 另一差异（T-e 外延段缺失）归因于交叉提取取了保守分支（连续区段式，不跨掩码空洞）
  加舍弃 terminal 边界放宽，与振幅无关。
...
- 裁决参数歧义：thick 初猜振幅按 stage 对齐产码（ternary 产的 stage 用 0.09/0.02，
  clean 产的用 0.3/0.3），或全局改 0.09/0.02——由用户定，定后重跑差异 case 验证归因。
- T-e 外延段：若要对齐，需决定是否补 clean 式跨空洞交叉提取或 terminal 放宽（机制
  取舍变更，须用户点头）。
- 上述解决后：rebuild-src 开 PR 合入 main（正文附验证摘要）。
- 遗留：12 case 档位不一致（400/150 混合）；case 3 参考线 phi1 延到 0.29 超出
  [0, 0.2] 扫描窗（project_plan 本就标注后期扩全区间）。
