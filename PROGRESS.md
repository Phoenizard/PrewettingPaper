...
- 阶段：验证收官，转入数据分析（服务论文正文）。12 case 验证 + 人工比对完成后，
  用户裁决：代码结果正确，具备独立复现 unvalidate_data 的能力，不需要重跑。
  rebuild-src 分支已经 PR #2 合入 main（2026-07-08）。
- 验证结论存档：12 case 中 8 个与对照图一致；4 个差异均已归因、判定非模型/求解器
  错误，按用户裁决不再处理——thick 初猜振幅参数歧义（两参考码 0.09/0.02 vs 0.3/0.3，
  我们全局 0.3/0.3，致 T-f 两 case 0 点、T-b 线中段缺失；机制与证据见 2026-07-08
  深夜日志）；T-e 外延段缺失归因交叉提取保守分支 + 舍弃 terminal 放宽，与振幅无关。
- 数据分析已开工（2026-07-09，未提交 git）：角度 1——omega1/omega2 如何影响
  prewetting line。产出 doc/analysis/angle1_omega.md（相图 + 物理读法 + 一句话结论）、
  src/pwpix.py（从对照 overlay PNG 颜色无关提取 PW 线像素，补数值 CSV 缺位）、
  scripts/angle1_omega.py（T-a 全 11x11 omega 网格提取 phi2_inf_max 指标并出图）。
...
- 等用户布置今日任务（方向已转数据分析，角度 1 为首个样例）。
- 待入 git：doc/analysis/、src/pwpix.py、scripts/angle1_omega.py（2026-07-09 分析
  产物，当前未追踪）。
