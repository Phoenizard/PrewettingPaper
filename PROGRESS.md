...
核心问题（2026-07-12 重定，留给新 session 专门讨论）：
对一个 case 的图，如何衡量 pre-wetting 容易发生的程度——即线的整体长度 + 线到
binodal 的关系（平均/最大距离）。关键决定：不区分 branch，一个 case 的所有
pre-wetting 点（多 branch 也一样）全部合并当一个整体来量。理由见下方今日日志：
两条 line 合拢/分开本身就是物理效果，按 branch 切开反而割裂它。

- 待解难题（这个思路引入的）：多分支/多段线情形下，整体长度怎么量、到 binodal 的
  整体距离怎么量。单直线拟合对多段线不成立（几何拟合已翻车），需要新定义。
- 前置：这个量要通用（4 个 topic 共用），先在 omega topic（chibb=0）上把定义敲定。
- 已放弃的路径：重算 gamma / cs 判 branch——那是被"必须区分 branch"这个错误前提
  逼出来的，前提去掉即不需要。相关代码 scripts/branch_by_gamma.py、
  src/geom.py 的 split_branches 保留存档，但不是当前主线。
